...
function mostrarMensaje(data) {
    const remitente = data.uniqueId;
    const repeatCount = data.repeatCount || data.comment;
    const giftPictureUrl = data.giftPictureUrl || data.profilePictureUrl;

    const sendGift = document.getElementById("sendGift");
    sendGift.innerHTML = generarMensajeHTML(remitente, repeatCount, giftPictureUrl);
    console.log("Elemento mostrado",sendGift.innerHTML);
}

function generarMensajeHTML(remitente, repeatCount, giftPictureUrl) {
    return `<link href='https://fonts.googleapis.com/css?family=Oswald' rel='stylesheet' 
    type='text/css'><center><h1 style="text-align: center;font-family: 'Oswald', Helvetica, 
    sans-serif;font-size: 80px;transform: skewY(-10deg);letter-spacing: 4px;word-spacing: -8px;color: 
    tomato;text-shadow: -1px -1px 0 firebrick, -2px -2px 0 firebrick, -3px -3px 0 firebrick, -4px -4px 0 firebrick, -5px -5px 0 firebrick,
     -6px -6px 0 firebrick, -7px -7px 0 firebrick, -8px -8px 0 firebrick, -30px 20px 40px dimgrey ">${remitente}   <br> ${repeatCount} 
     <img src='${giftPictureUrl}'></h1></center>`;
}
  ...
  function overlayTime(eventType, data) {
    ...
    switch (eventType) {
        ...
        case 'gift':
            ...
            userNickname = data.nickname;
            userUniqueName = data.uniqueId;
            userGigtname = data.giftPictureUrl;
